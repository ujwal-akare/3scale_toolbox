RSpec.describe 'Application apply command' do
  ...
  let(:command_line_str) { "application apply --suspend #{client_url} #{application_id}" }
  ...
  it 'application is suspended' do
    ...
    expect(application.attrs['state']).to eq('suspended')
  end
end
