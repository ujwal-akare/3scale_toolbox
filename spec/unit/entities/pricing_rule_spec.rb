RSpec.describe ThreeScaleToolbox::Entities::PricingRule do
  ...
  context 'instance method' do
    ...
    let(:pr) { described_class.new(id: pr_id, plan: plan, metric_id: metric_id, attrs: attrs) }
    subject { pr }
    ...
    context '#to_cr' do
      ...
      it 'metricMethodRef included' do
        expect(subject).to include('metricMethodRef' => { 'systemName' => metric_1.system_name })
      end
    ...
    context '#backend_metric' do
      ...
      context 'when no backend link' do
        let(:attrs) { { 'cost_per_unit' => '0.0', 'min' => 1, 'max' => 12 } }
        it do
          is_expected.to be_nil
        end
      end
    ...
    context '#to_hash' do
      ...
      let(:attrs) do
        {
          'cost_per_unit' => '0.0', 'min' => 1, 'max' => 12, 'metric_id' => metric_id,
          'links' => [
            {
              'rel' => 'metric',
              'href' => "https://example.com/admin/api/backend_apis/#{backend_id}/metrics/#{metric_id}"
